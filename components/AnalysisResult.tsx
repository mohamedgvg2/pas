...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ imageUrl, feedback, onProceed, onUploadNew }) => {
  return (
    <div className="w-full flex flex-col items-center animate-fade-in">
        ...
        <p className="text-center text-slate-600 dark:text-slate-400 mb-6 max-w-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 w-full">
        <div className="text-center">
          <h3 className="text-xl font-semibold mb-3 text-slate-600 dark:text-slate-400">صورتك الحالية</h3>
        ...
        <div>
            <h3 className="text-xl font-semibold mb-3 text-slate-600 dark:text-slate-400">الملاحظات</h3>
      ...
      <div className="mt-10 flex flex-col sm:flex-row gap-4 w-full max-w-md">
        <button
          onClick={onUploadNew}
          className="w-full text-center bg-slate-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-slate-700 transition-colors duration-300 shadow-md shadow-slate-200/80 dark:shadow-none focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 text-lg"
        >
          تحميل صورة جديدة
        </button>
        <button
          onClick={onProceed}
          className="w-full text-center bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-indigo-700 transition-colors duration-300 shadow-md shadow-indigo-200/80 dark:shadow-none focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-lg"
        >
          المتابعة على أي حال
        </button>
      </div>
